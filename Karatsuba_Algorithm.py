...
def num2list(number):
# num2list transforms an integer into a list of it digits
    list_of_digits = []
    while number > 0:
...
def list2num(list_of_digits):
# list2num transforms a list of it digits into a number
    number = 0                                      #TO BE DISCUSSED how to create number = None of the type integer
    for i in range (len(list_of_digits)):
        number += list_of_digits[-1-i]*(10**i)
    return number

def downstream_simple (number_1, number_2):
    split_index = int(len(num2list(number_1))/2)
    list_1, list_2 = num2list (number_1), num2list (number_2)
    ...
    number_1_front, number_1_back = list2num(list_1_front), list2num(list_1_back)
    number_2_front, number_2_back = list2num(list_2_front), list2num(list_2_back)       
...
def karatsuba_algorithm (number1, number2):
    karatsuba_product = upstream_recursive (upstream_base (downstream_recursive (input_numbers(number1, number2))))
